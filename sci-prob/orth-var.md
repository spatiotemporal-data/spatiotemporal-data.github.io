...
To capture interpretable modes/patterns, one can use tensor factorization formulas to parameterize the coefficients and the optimization problem can be easily built. However, one great challenge would be how to make the modes "more interpretable". In this post, we propose an optimization problem of the time-varying autoregression as follows,
